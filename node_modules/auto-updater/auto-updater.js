...
module.exports = function( opciones ){
	...
	AutoUpdater._loadRemoteJson = function(){
		...
		this._remoteDownloader({host:'raw.githubusercontent.com',path:path},function(data){ //hostname fixed
	...
	AutoUpdater._loaded = function(){
		if ( parseInt(this.jsons.client.version.split('.').join(''), 10) >= parseInt(this.jsons.remote.version.split('.').join(''), 10) ) {		//my fix for maintaners without .git folder
