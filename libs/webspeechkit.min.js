(function(window){window.webspeechkit={recorderInited:false,FORMAT:{PCM8:{format:'pcm',samplerate:8000,mime:"audio/x-pcm;bit=16;rate=8000"},PCM16:{format:'pcm',samplerate:16000,mime:"audio/x-pcm;bit=16;rate=16000"},PCM44:{format:'pcm',samplerate:44100,mime:"audio/x-pcm;bit=16;rate=44100"},SPEEX8:{format:'speex',samplerate:8000,mime:"audio/x-speex"},SPEEX16:{format:'speex',samplerate:16000,mime:"audio/x-speex"}}};var ac;var scriptPath=function(){var scripts=document.getElementsByTagName('script');var path='';if(scripts&&scripts.length>0){for(var i in scripts){if(scripts[i].src&&scripts[i].src.match(/\/webspeechkit.min.js$/)){path=scripts[i].src.replace(/(.*)\/webspeechkit.min.js$/,'$1');break;}}}
...
{var backref=this;this.bufferLen=bufferSize||4096;this.channelCount=Math.max(1,Math.min(channelCount||2,2)); this.context=ac||new AudioContext();ac=this.context;this.outSampleRate=outSampleRate||this.context.sampleRate
